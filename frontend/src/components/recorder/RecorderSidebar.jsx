...
function NavItem({ item, active, badgeCount, badgeColor, isCollapsed, onClick }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ 
        ...
        opacity: isCollapsed ? 0 : 1, width: isCollapsed ? 0 : 'auto', transition: 'opacity 0.2s ease, width 0.2s ease'
      }}>
        {item.label}
      </span>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <svg width="24" height="24" viewBox="0 0 26 26" className="logo-glow" style={{ flexShrink: 0 }}>
        ...
        <div style={{ 
          ...
          opacity: isCollapsed ? 0 : 1, transition: 'opacity 0.15s ease', pointerEvents: isCollapsed ? 'none' : 'auto',
          display: 'flex', flexDirection: 'column'
        }}>
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '1.5px', color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>
            HYBRID SENTRY
          </span>
      ...
      <div
        ...
        title={isCollapsed ? `${name} - Click to logout` : "Click to logout"}
